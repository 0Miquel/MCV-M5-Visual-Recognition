...
class Logger:
    def __init__(self, cfg, wandb_name):
        ...
        if cfg is not None:
            ...
            self.run = wandb.init(project=self.project_name, config=self.cfg, entity="bipulantes")
        else:
            # If called by wandb.agent,
            # this config will be set by Sweep Controller
            self.sweep = True
            self.run = wandb.init(project=self.project_name, config=cfg, entity="bipulantes")
            config = wandb.config
