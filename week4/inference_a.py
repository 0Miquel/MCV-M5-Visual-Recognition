...
def infere_dataset(dataset, model, cfg, last_layer_size=512):
    data = np.empty((len(dataset), last_layer_size))
    with torch.no_grad():
        model.eval()
        for ii, (img, _) in tqdm(enumerate(dataset)):
            img = img.to(cfg["device"])
            data[ii, :] = model(img.unsqueeze(0)).squeeze().cpu().numpy()
    return data
...
def main(cfg):
    ...
    # set inference model
    inference_model = HeadlessResnet(cfg["weights_path"]).to(cfg["device"])
    ...
    catalogue_data = infere_dataset(train_dataset, inference_model, cfg)
    query_data = infere_dataset(val_dataset, inference_model, cfg)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--config', default='configs/inference_a.yaml')
